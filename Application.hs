...
mkYesodDispatch "WebProxy" resourcesWebProxy
...
withWebProxy :: AppConfig -> Logger -> (Application -> IO a) -> IO a
withWebProxy conf logger f = do
#ifdef PRODUCTION
    s <- static Settings.staticDir
#else
    s <- staticDevel Settings.staticDir
#endif
    let h = WebProxy conf logger s
    toWaiApp h >>= f
...
withDevelAppPort =
  ...
  where
    ...
    go f = do
        ...
        withWebProxy conf logger $ \app -> f (port, debugHandle (logHandle logger) app)
        flushLogger logger
      where
